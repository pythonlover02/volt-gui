...
const BLOCKED_LINE: &str =
    "anisotropy: asked off; the application did not enable samplerAnisotropy";
...
fn restores_a_list_the_choice_emptied() {
    assert_eq!(
        kept(vec![1, 2, 3], |value: &i32| *value > 3, LIST_EMPTY_WARN),
        vec![1, 2, 3]
    );
    assert_eq!(
        kept(vec![1, 2, 3], |value: &i32| *value > 1, LIST_EMPTY_WARN),
        vec![2, 3]
    );
}
...
fn keeps_only_what_the_choice_names() {
    assert_eq!(
        filtered(vec![1, 2, 3], Some(2), |v: &i32| Some(*v), LIST_EMPTY_WARN),
        vec![2]
    );
    assert_eq!(
        filtered(vec![1, 2, 3], None, |v: &i32| Some(*v), LIST_EMPTY_WARN),
        vec![1, 2, 3]
    );
    assert_eq!(
        filtered(vec![1, 2, 3], Some(9), |v: &i32| Some(*v), LIST_EMPTY_WARN),
        vec![1, 2, 3]
    );
}
...
fn round_trips_a_present_mode_through_its_name() {
    ...
    assert_eq!(
        present_parse(&present_display(UNKNOWN_MODE)),
        Some(UNKNOWN_MODE)
    );
}
...
fn round_trips_a_composite_alpha_through_its_name() {
    assert_eq!(
        alpha_parse(&alpha_display(OPAQUE_ALPHA)),
        Some(OPAQUE_ALPHA)
    );
    assert_eq!(
        alpha_parse(&alpha_display(UNKNOWN_ALPHA)),
        Some(UNKNOWN_ALPHA)
    );
}
...
fn reads_the_facts_a_setting_branches_on() {
    assert_eq!(
        alpha_semantic(OPAQUE_ALPHA).map(|facts| facts.blends),
        Some(false)
    );
    assert_eq!(
        alpha_semantic(INHERIT_ALPHA).map(|facts| facts.blends),
        Some(true)
    );
    assert_eq!(
        present_semantic(FIFO_MODE).map(|facts| facts.extended),
        Some(false)
    );
    assert_eq!(
        present_semantic(SHARED_MODE).map(|facts| facts.extended),
        Some(true)
    );
}
...
fn forces_a_value_volt_has_no_name_for() {
    assert_eq!(
        parse_settings(UNNAMED_MODE_PROFILE).present_mode,
        Some(UNKNOWN_MODE)
    );
    assert_eq!(
...
fn reads_the_two_feature_gated_toggles_from_a_profile() {
    assert_eq!(
        parse_settings(ALPHA_ONE_PROFILE).alpha_to_one,
        Some(TOGGLE_ON)
    );
...
fn shifts_the_frame_limit_by_the_offset() {
    assert_eq!(
        shifted_fps(REFRESH_FPS, Some(OFFSET_DOWN)),
        UNDER_REFRESH_FPS
    );
...
fn never_shifts_a_cap_below_one_frame_a_second() {
    assert_eq!(
        shifted_fps(FRAME_LIMIT_MIN, Some(OFFSET_MIN)),
        FRAME_LIMIT_MIN
    );
}
...
fn reads_a_frame_limit_offset_from_a_profile() {
    assert_eq!(
        parse_settings(OFFSET_PROFILE).frame_limit_offset,
        Some(OFFSET_DOWN)
    );
}
...
fn reads_a_frame_limit_cadence_from_a_profile() {
    assert_eq!(
        parse_settings(FIXED_PROFILE).cadence,
        Some(CadenceChoice::Fixed)
    );
    assert_eq!(
        parse_settings(DYNAMIC_PROFILE).cadence,
