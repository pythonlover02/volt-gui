...
def parse_device_name_from_raw(raw_text):
    if "/" in raw_text: return raw_text.split("/")[0].strip().lower()
    if "(" in raw_text: return raw_text.split("(")[0].strip().lower()
    return raw_text.lower()


def build_opengl_environment(extra_environment):
...
def parse_opengl_device_from_output(command_output):
...
def build_deduplicated_device_names(detected_devices):
    seen = {}
...
def build_device_map_from_detected(detected_devices):
...
def build_deduplicated_device_result(detected_devices):
    return {"devices": build_deduplicated_device_names(detected_devices), "device_map": build_device_map_from_detected(detected_devices)}


def find_opengl_devices_sync():
    ...
    for mesa_device in mesa_devices:
        all_detected.append(mesa_device)
...
def process_opengl_detection_worker(completion_callback):
...
def process_opengl_detection_async(completion_callback):
    threading.Thread(target=lambda: process_opengl_detection_worker(completion_callback), daemon=True).start()
    return None


def get_display_name_for_vulkan_device(raw_device_name):
...
def build_vulkan_device_identifier_string(accumulated_properties):
    return accumulated_properties["vendorID"] + ":" + accumulated_properties["deviceID"]


def parse_vulkan_device_entries(command_output):
    ...
    for output_line in command_output.split("\n"):
        for property_name in ("vendorID", "deviceID", "deviceName"):
            ...
            result_entries.append((get_display_name_for_vulkan_device(accumulated_properties["deviceName"]), build_vulkan_device_identifier_string(accumulated_properties)))
            accumulated_properties = {}
    return tuple(result_entries)


def find_vulkan_devices_sync():
...
def process_vulkan_detection_worker(completion_callback):
