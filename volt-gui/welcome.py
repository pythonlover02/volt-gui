...
def get_welcome_settings() -> dict:
    return {
        ...
        "Presets": {
            "Presets": (
                ("text", "Presets fill the profile you have open with a starting point, arranged as a ladder from best looking to fastest:\n\n- Quality: trilinear filtering, blended mips, 16x anisotropy, a slight sharpening bias, every mip level allowed, full sample shading, smoothed cutout edges, classic vsync, a 4 image swapchain, 10 bit colour where the surface has it, and precise pacing on an early wait.\n- Balanced: trilinear and blended mips still, 8x anisotropy, a little over half sample shading, mailbox present for vsync without the latency, precise pacing.\n- Performance FPS: bilinear, 4x anisotropy, a blurring bias, no sample shading, mailbox present, the swapchain held to 4 images, colour held to 8 bit and the cheaper sleep pacing.\n- Performance Low Latency: the same, aimed at input lag instead, with immediate present, a 2 image swapchain, a late wait and precise pacing.\n- Potato FPS: bilinear, anisotropy off, hard mip cuts, a full step of blurring bias, the top two mips off the table, cutout smoothing off.\n- Potato Low Latency: the same again with immediate present, a 2 image swapchain and a late wait."),
