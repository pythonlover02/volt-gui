...
def get_welcome_settings() -> dict:
    return {
        ...
        "Presets": {
            "Presets": (
                ("text", "Presets fill the profile you have open with a starting point, arranged as a ladder from best looking to fastest:\n\n- Quality: trilinear filtering, blended mips, 16x anisotropy, a slight sharpening bias, full sample shading, smoothed cutout edges, classic vsync and a 4 image swapchain.\n- Balanced: trilinear and blended mips still, 8x anisotropy, a little over half sample shading, mailbox present for vsync without the latency.\n- Performance FPS: bilinear, 4x anisotropy, a blurring bias, no sample shading, mailbox present and the swapchain held to 4 images.\n- Performance Low Latency: the same, aimed at input lag instead, with immediate present and a 2 image swapchain.\n- Potato FPS: bilinear, anisotropy off, hard mip cuts, a full step of blurring bias, cutout smoothing off.\n- Potato Low Latency: the same again with immediate present and a 2 image swapchain."),
