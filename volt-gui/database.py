...
SETTINGS_DB: Final[dict] = {
    ...
    "Display": {
        ...
        "color_depth": {
            "section": "display",
            "label": "Color Depth",
            "description": "Reorder the surface formats the game sees when it queries the surface. 10bit prefers 10-bit SDR formats, hdr10 prefers HDR10 output with metadata, scrgb prefers extended linear sRGB. Games that pick the first supported format follow the preference; hardcoded choices are respected. hdr10 and scrgb need driver and compositor support and are skipped without it.",
            "options": (DEFAULT_VALUE, "10bit", "hdr10", "scrgb"),
            "editable": False,
        },
    },
    "Framerate": {
        ...
        "frame_pacing": {
            "section": "framerate",
            "label": "Frame Pacing",
            "description": "How the frame limiter waits. sleep is CPU friendly, precise sleeps most of the interval then busy waits the remainder for tighter frametimes at a small CPU cost. display_timing paces like precise today and is reserved for compositor timed pacing. Only applies when Frame Limit is set.",
            "options": (DEFAULT_VALUE, "sleep", "precise", "display_timing"),
            "editable": False,
        },
        "anti_lag": {
            "section": "framerate",
            "label": "AMD Anti-Lag",
            "description": "Reduce input latency on AMD GPUs through the driver's anti lag support. Needs the VK_AMD_anti_lag device extension; skipped without it. When Frame Limit is set the same cap is passed to the driver.",
            "options": (DEFAULT_VALUE, "on", "off"),
            "editable": False,
        },
        "low_latency": {
            "section": "framerate",
            "label": "NVIDIA Low Latency",
            "description": "Reduce input latency on NVIDIA GPUs through the driver's low latency mode. boost additionally raises GPU clocks under CPU bound loads. Needs the VK_NV_low_latency2 device extension; skipped without it.",
            "options": (DEFAULT_VALUE, "on", "boost"),
            "editable": False,
        },
    },
    "Textures": {
        ...
        "lod_min": {
            "section": "textures",
            "label": "Minimum LOD",
            "description": "Lowest mip level samplers may use. Raising it forces smaller mips everywhere, trading detail for speed. On drivers with VK_EXT_image_view_min_lod this also applies at the image view level, catching games the sampler path misses.",
